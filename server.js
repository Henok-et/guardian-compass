...
async function initializeGoogleSheets() {
    try {
        ...
        // We still need service account credentials
                ...
                get: async () => ({
                    data: {
                        values: [
                            ...
                            [new Date().toISOString(), "AU Youth Verification", "Configuring OAuth Client ID..."]
                        ]
                    }
                })
            }
    ...
    res.json({
        ...
        configuration: {
            ...
            oauthClientIdPreview: hasOAuthClient ? 
                process.env.GOOGLE_CLIENT_ID.substring(0, 25) + "..." : "N/A",
...
app.get("/api/setup-instructions", (req, res) => {
    ...
    res.json({
        ...
        currentStatus: {
            ...
            authenticationMethod: hasOAuthClient ? "OAuth Client ID" : 
                                 hasApiKey ? "API Key" : 
                                 hasCredentials ? "Service Account" : 
                                 "None (Demo Mode)"
        },
        ...
        steps: [
            {
                ...
                details: "In Google Cloud Console → Credentials → Create OAuth Client ID"
            },
            {
                step: 2,
                action: "Add to Render.com",
                details: "Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET environment variables"
            },
            {
                step: 3,
